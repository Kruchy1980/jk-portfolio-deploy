...
//     let navigationLinks = [...document.querySelectorAll('.main-navigation__menu-item')];
...
const activeLinks = event => {
    ...
    let topPart = window.scrollY;
    // console.log(topPart);
    navigationLinks.forEach(link => {
        let section = document.querySelector(link.hash);
        // console.log(section);
        if (section.offsetTop <= topPart && section.offsetTop + section.offsetHeight > topPart) {
            link.classList.add('active--links');
        } else {
            link.classList.remove('active--links');
        }
